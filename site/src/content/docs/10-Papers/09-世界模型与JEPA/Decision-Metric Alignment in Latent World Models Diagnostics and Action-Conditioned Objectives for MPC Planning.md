...
- 谱系锚：[世界模型](/explore/20-Algorithms/世界模型)，[MCTS置信上界](/explore/30-Formulas/MCTS置信上界)
...
- 工程域外部证据（260915）：[WAM 挖掘](/explore/10-Papers/09-世界模型与JEPA/From Prediction to Decision - World-Model-Guided Action Selection for Continuous Pile Excavation（挖掘WAM）)——固定提案器、只换选择规则的消融把 WM 价值落成决策级数字（铲数 −17.1%）；"预测保真≠决策保真"的机器人域实例，E1 动机节可并列引用。
