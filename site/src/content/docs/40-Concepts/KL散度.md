...
代入 $$\mathcal{N}(1.5, 2)\|\mathcal{N}(1, 1)$$：$$\log\frac{1}{\sqrt2} + \frac{2+0.25}{2} - 0.5 = -0.347+1.125-0.5 = 0.278$$（与 [ELBO](/ai-fa/explore/40-Concepts/ELBO) §2 第 6 步的 gap 手算同一道题——**ELBO 的 gap 就是这个公式**）。VAE 的 KL 正则项=它的对角特例（[变分自编码器](/ai-fa/explore/20-Algorithms/变分自编码器) §3 第 4 步）。
