...
**第 1 步：score 是什么。** $$s(x) = \nabla_x\log p(x)$$——**概率地形的坡度指向**（哪里数据密集往哪指）。弹簧直觉与两步朗之万手算见 [Score函数](/ai-fa/explore/40-Concepts/Score函数)。

**第 2 步：训练（DSM）。** 加噪后目标闭式：$$s^* = -(\tilde x - x_0)/\sigma^2$$，回归网络输出（[DSM目标](/ai-fa/explore/30-Formulas/DSM目标) 教程：−0.8 的标签现场、与 ε-预测差 σ 倍的换算）。

**第 3 步：采样（退火朗之万）。** $$x \leftarrow x + \frac{\eta}{2}s_\theta + \sqrt\eta\,\epsilon$$——大 σ 粗定位、小 σ 精修（L 档噪声链，[DSM目标](/ai-fa/explore/30-Formulas/DSM目标) §采样）。

**第 4 步：连续化（Score-SDE）。** 噪声档→无穷细 = SDE；反向方程只需把 score 塞进漂移项（[反向SDE](/ai-fa/explore/30-Formulas/反向SDE) 教程第 4 步：修正项 −g²s 就是"拽回流形的手"）；去噪声版 = ODE（[概率流ODE](/ai-fa/explore/30-Formulas/概率流ODE)）。

**第 5 步：读法。** score 派（SMLD）与扩散派（DDPM）两族论文用两套记号干了同一件事（s = −ε/σ），Score-SDE 把它们连同 VE/VP 两个家族统一进一个框架（[Score-SDE前向过程](/ai-fa/explore/30-Formulas/Score-SDE前向过程) 教程：欧拉一步 2.037 vs DDPM 2.036 的泰勒对账）——**本卡是这条统一史的主线叙事，数字全在各公式卡里**。
