...
## 5. 与前作/矩阵关系
- 线锚：[[40-Concepts/策略梯度定理]]（CROP=Critic 引导相对离线策略优化，GRPO 家族的离线安全化变体） · [[10-Papers/04-强化学习与对齐/Planning with Diffusion for Flexible Behavior Synthesis（Diffuser）|Diffuser]]（同族：生成式序列决策——DT 谱系的轨迹条件建模）
- 同轴：[[10-Papers/04-强化学习与对齐/Understanding R1-Zero-Like Training- A Critical Perspective（Dr.GRPO）|Dr.GRPO]]（组相对打分的偏差对照——CROP 用 critic 打分替代在线 rollout 的组采样） · [[40-Concepts/过程奖励与结果奖励（PRM-ORM）]]（价值感知正则的打分层视角）
