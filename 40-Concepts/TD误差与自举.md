...
**第 1 步：设定。** 玩具宇宙（[[40-Concepts/马尔可夫决策过程]]）真值 $V^\pi(s_1) = 0.870$，但 critic 初值 $\hat V = 0.5$（还没学）。
...
**第 4 步：δ = 优势（TD(0) 恒等式）。** 用真值代入：$\delta(a_{慢}) = 0.9\times0.870-0.870 = -0.087 = A(a_{慢})$；$\delta(a_{赢}) = 1-0.870 = +0.130 = A(a_{赢})$——**一步的惊讶恰好等于"这步比平均好多少"**（一般多步情形两者不同，GAE 才是正式桥梁，[[40-Concepts/广义优势估计GAE]]）。
