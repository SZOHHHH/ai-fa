...
## 5. 与前作/矩阵关系
- 线锚：[[40-Concepts/视觉语言模型（VLM）]]（评测对象=MLLM 长视频推理）· [[40-Concepts/注意力机制]]（"证据-上下文权衡"=注意力稀释的基准化表述）
- 同族：↔ [[10-Papers/06-长上下文/Dodo- Dynamic Contextual Compression for Decoder-only LMs（Dodo）|Dodo]]（token 压缩同动机：本卡音-文本引导蒸馏高价值推理线索，长视频版"该留哪些 token"，260919 处理段挂链，占位层待精读）
