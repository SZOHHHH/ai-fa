...
- **自我引导循环**：采样 [CoT](/explore/10-Papers/07-推理模型/Chain-of-Thought Prompting Elicits Reasoning in Large Language Models（CoT）) → 保留对的 → SFT → 更强 → 再采样
...
- 与 [GRPO与RLVR](/explore/20-Algorithms/GRPO与RLVR) 的规则奖励同思想（早三年）

## 5. 与前作的关系

- 前置于 [Chain-of-Thought Prompting Elicits Reasoning in Large Language Models](/explore/10-Papers/07-推理模型/Chain-of-Thought Prompting Elicits Reasoning in Large Language Models（CoT）)：CoT 有了，STaR 让 CoT 进入模型权重
...
[思维链（CoT）](/explore/40-Concepts/思维链（CoT）)、[Chain-of-Thought Prompting Elicits Reasoning in Large Language Models](/explore/10-Papers/07-推理模型/Chain-of-Thought Prompting Elicits Reasoning in Large Language Models（CoT）)、[GRPO目标](/explore/30-Formulas/GRPO目标)
