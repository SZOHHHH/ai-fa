...
## 5. 与前作/矩阵关系
- 线锚：[KV缓存](/ai-fa/explore/40-Concepts/KV缓存)（全局注意力=每步全量读 KV，本卡主战场）· [稀疏与线性注意力](/ai-fa/explore/40-Concepts/稀疏与线性注意力)（local-first 解码=稀疏化家族的二值极端：每步在 local/full 间二选一）
- 同族：↔ [MoBA](/ai-fa/explore/10-Papers/06-长上下文/MoBA- Mixture of Block Attention for Long-Context LLMs（MoBA）)（学"读全局哪些块"，本卡学"要不要读全局"——收益预测的路线分岔，260919 处理段挂链，占位层待精读）· [StreamingLLM](/ai-fa/explore/10-Papers/06-长上下文/Efficient Streaming Language Models with Attention Sinks（StreamingLLM）)（local 窗+sink 即 ODA 的纯 local 极端基线）
