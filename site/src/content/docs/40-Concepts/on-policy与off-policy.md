...
**第 5 步：PPO 的位置。** PPO 每轮小步更新后立刻重采——$$\pi_\theta$$ 与 $$\pi_{old}$$ 始终接近（比率 ≈ 1）→ 修正温和、方差可控；万一某方向冲太远，裁剪兜底（四象限见 [PPO裁剪目标](/ai-fa/explore/30-Formulas/PPO裁剪目标) 教程）。这就是"近 on-policy"的设计逻辑：**不追求 off 的样本效率，换取 on 的稳定性**。
