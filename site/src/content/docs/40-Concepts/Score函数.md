...
**第 5 步：score 与 ε 预测的关系（DDPM 的另一张脸）。** $$q(x_t\mid x_0)=\mathcal{N}(\sqrt{\bar\alpha_t}x_0,\ 1-\bar\alpha_t)$$，按第 1 步同法求 score：$$\nabla_{x_t}\log q = -\frac{x_t-\sqrt{\bar\alpha_t}x_0}{1-\bar\alpha_t} = -\frac{\epsilon}{\sqrt{1-\bar\alpha_t}}\cdot\frac{1}{\sqrt{1-\bar\alpha_t}} = -\frac{\epsilon}{1-\bar\alpha_t}$$——**预测 score ⇔ 预测所加噪声**（差尺度因子）——同一枚硬币，DDPM 训练目标（[DDPM训练目标](/ai-fa/explore/30-Formulas/DDPM训练目标)）与 denoising score matching 在这里合流。
