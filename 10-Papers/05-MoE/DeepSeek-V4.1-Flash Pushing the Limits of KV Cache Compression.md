...
## 5. 与前作/矩阵关系
- 线锚：[[20-Algorithms/混合专家（MoE）]] · [[30-Formulas/MLA多头潜在注意力]]（KV 压缩谱系）· [[40-Concepts/KV缓存]]（本卡主战场概念）
- 同族：← [[10-Papers/05-MoE/DeepSeek-V3 Technical Report（DeepSeek-V3）|DeepSeek-V3]]（MLA+MoE 前代旗舰，本卡=KV 压缩极限推演）· [[10-Papers/05-MoE/DeepSeekMoE- Towards Ultimate Expert Specialization in Mixture-of-Experts Language Model（DeepSeekMoE）|DeepSeekMoE]]（260918 处理段挂链，占位层待精读）
- 同族（续，260919）：↔ [[10-Papers/06-长上下文/Physically Partitioned KVCache Format for CPU--GPU Load Balancing in MoE Inference|InplaceKVCache/WriteScope]]（同攻 MoE 长上下文 KV 部署：V4.1-Flash 压 KV 体积，WriteScope 做单卡 CPU-GPU 物理布局与负载均衡——压缩轴与调度轴互补）
