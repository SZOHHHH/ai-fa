...
|---|---|---|
| 确定动态（球类物理） | 尖峰（接近 one-hot） | 果几乎唯一锁定因——反推可靠 |
...
- **软输出才是宝**：用 [[30-Formulas/交叉熵|交叉熵]] 训练的 IDM，其总体最优=匹配真后验 $p(a\mid s,s')$（交叉熵=KL+熵的恒等式直接给出）——18 维概率向量本身就是"哪些按键都行"的完整答案，argmax 只是它的一个投影。
