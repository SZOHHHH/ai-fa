...
**第 1 步：成对偏好 $(y^+, y^-)$ + 有参考模型预算。** → **DPO**（[[30-Formulas/DPO损失]] 教程：初始 0.693、梯度权重自适应）；担心过优化（y⁻ 被无底线压）→ **IPO**（[[30-Formulas/IPO损失]]：目标 $\frac{1}{2\beta}$ 达标即停 vs DPO 渐近追分）。

**第 2 步：成对 + 想省显存。** → **SimPO**（[[30-Formulas/SimPO损失]]：长度归一防判定翻转 + γ 间距，无参考模型省一半前向）或 **ORPO**（[[30-Formulas/ORPO损失]]：odds 比率 + SFT 锚一锅出——连独立 SFT 阶段都省）。

**第 3 步：只有点赞点踩单标签。** → **KTO**（[[30-Formulas/KTO损失]]：好抬坏压镜像 + 前景理论权重 λ）——现实用户反馈生态的直连。

**第 4 步：只想在线探索（有可验证奖励）。** → GRPO/RLVR 线（[[30-Formulas/GRPO目标]]：组内 ±1 优势 + 规则奖励，不走离线偏好）。

**第 5 步：共同血缘。** 全家共用一条推导链：RLHF 闭式解（[[30-Formulas/RLHF目标]] §2）→ 奖励=策略比表达 → BT/前景理论似然——**选型只是选"信哪种数据、留哪段流水线"**。
