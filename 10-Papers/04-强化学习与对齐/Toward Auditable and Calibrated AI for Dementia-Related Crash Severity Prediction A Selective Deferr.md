...
- 评测学表亲（跨线）：[[Decision-Metric Alignment in Latent World Models Diagnostics and Action-Conditioned Objectives for MPC Planning]]——同属"预测指标≠决策效用"家族：那边是潜距离不保任务排序，这边是准确率不保分诊工作流（漏分代价不对称）；两枚跨域证据可并列引用
- 基线谱系：[[BERT- Pre-training of Deep Bidirectional Transformers for Language Understanding（BERT）]]（BERT 系基线）与 Gemma/本地 LLM 基线（07-推理模型线交叉）
- 数学根基：[[30-Formulas/交叉熵]]（校准视角下的损失）· [[40-Concepts/温度参数]]（置信度校准的标配工具）· [[40-Concepts/条件概率]]（ECE=分箱内置信对精度的条件平均偏差）
...
[[30-Formulas/交叉熵]]（分类损失与置信度）· 期望校准误差 ECE（分箱置信-精度差的均值，建议先理解可靠性图）· 选择性预测与拒绝学习（Geifman & El-Yaniv 一系的基本设定）· 结果泄漏（decision-time 不可得信息混入训练特征——特征工程里最隐蔽的作弊）。
