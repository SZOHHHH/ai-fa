...
输出是 $d$ 个 0~1 的"旋钮"。读到新词"但是"→ 旋钮集体拧向 0（前文要反转了，旧状态该清）；读到"换句话说"→ 拧向 0.5（要改写但保留骨架）。**它就是 [[30-Formulas/选择机制|Mamba 选择门]]的直系祖先**：$\odot$ 门控状态线，一脉相承。

**第 2 步：输入门+候选——新货是什么、装多少？**
$$i_t = \sigma(W_i[h_{t-1},x_t]+b_i)\ (\text{装多少}), \qquad \tilde{c}_t = \tanh(W_c[h_{t-1},x_t]+b_c)\ (\text{候选新内容}\in(-1,1)^d)$$
分工：候选用 [[40-Concepts/激活函数族|tanh]]（**内容**，有正有负）；门用 sigmoid（**量**，0~1）。"写什么"与"写多少"解耦。
...
[[40-Concepts/sigmoid函数]]（三门的标准件——门控位至今不可替代的例子）、[[40-Concepts/激活函数族]]（tanh 做候选/门控分工）、[[40-Concepts/梯度]]（加法更新=梯度高速路的机制）、[[20-Algorithms/RNN]]（被治理的本体）、[[20-Algorithms/反向传播]]（BPTT 语境）、[[30-Formulas/残差连接]]（空间维的后裔对照）
