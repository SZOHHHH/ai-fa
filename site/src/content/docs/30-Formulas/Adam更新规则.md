...
- 谱系与选型见上位卡 [梯度下降与优化器谱系](/ai-fa/explore/20-Algorithms/梯度下降与优化器谱系)（本卡是其中的"第 5 级"）
...
| 表示名 | 公式核心 | 出处 | 说明 |
|---|---|---|---|
| Adam（本卡） | $$m, v$$ 双动量+偏差修正 | [Adam- A Method for Stochastic Optimization](/ai-fa/explore/10-Papers/01-架构演进/Adam- A Method for Stochastic Optimization（Adam）) | 默认 β=(0.9, 0.999) |
| **AdamW** | 衰减从梯度中解耦：$$\theta\leftarrow(1-\eta\lambda)\theta-\eta\hat m/(\sqrt{\hat v}+\epsilon)$$ | [Decoupled Weight Decay Regularization](/ai-fa/explore/10-Papers/01-架构演进/Decoupled Weight Decay Regularization（AdamW）) | LLM 标配——正则语义修复 |
...
- **AdamW 的教训**：把权重衰减混入梯度会被 $$\sqrt{\hat v}$$ **重新缩放**（衰减强度随参数漂移）；解耦后衰减按原意工作——一行改动，正则语义归位（[过拟合与正则化](/ai-fa/explore/40-Concepts/过拟合与正则化)）

## 5. 数学概念分解

- [梯度](/ai-fa/explore/40-Concepts/梯度)：$$g_t$$ 的来源（BP 算出）
- [期望](/ai-fa/explore/40-Concepts/期望)：$$m, v$$ 都是梯度（及其平方）的指数加权"期望估计"
- [梯度下降与优化器谱系](/ai-fa/explore/20-Algorithms/梯度下降与优化器谱系)：GD→SGD→Momentum→Adam 五级进化的上位叙事
- [过拟合与正则化](/ai-fa/explore/40-Concepts/过拟合与正则化)：权重衰减的正则身份

## 6. 与其他公式的关系

- ⊂ 属于 [梯度下降与优化器谱系](/ai-fa/explore/20-Algorithms/梯度下降与优化器谱系)（含五级手算进化史）
- ↔ 对照 [Decoupled Weight Decay Regularization](/ai-fa/explore/10-Papers/01-架构演进/Decoupled Weight Decay Regularization（AdamW）)：衰减项的位置（进梯度 vs 出梯度）
- ↔ 一切训练循环的"消费者"：[反向传播](/ai-fa/explore/20-Algorithms/反向传播) 算梯度 → 本公式用梯度
