...
- → 后继补记（260922）：[[WorldCrafter Consistent Video World Model with Implicit 3D-aware Memory]]——"透视+记忆 vs 显式 3D vs 全景潜状态"三角里的**隐式 3D 记忆**新代表：新视角重建预训练编码器写记忆+按目标视角读出固定预算 token（重访一致性 +47.6%）；仍是生成质量轴，无决策接口。
