...
**第 3 步：组内标准化。** mean = 0.5、std = 0.5 → $\hat A = (\pm1)$——**完整的 (1,1,0,0) 四样本手算见 [[30-Formulas/GRPO目标]] 教程**；全对/全错组优势全零（梯度消失的退化病与 CGE 修法同卡）。

**第 4 步：更新。** 组内每 token 走 PPO 裁剪（四象限见 [[30-Formulas/PPO裁剪目标]]）+ KL 锚住 SFT 参考防语言崩坏——**没有 critic 网络**（PPO 的 V 网络被 8 次采样替代：MC 基线，[[40-Concepts/贝尔曼方程]] 误区区）。

**第 5 步：涌现叙事。** R1-Zero 证明：跳过 SFT 的纯 RL + 规则奖励，模型自发长出反思/验证/长链行为——**可验证信号（数学/代码对错）足以驱动推理能力涌现**；蒸馏版再把大模型的长链能力转给小模型（[[40-Concepts/知识蒸馏]] §R1 行）。
