...
**第 3 步：读逐步价值序列。** $$(0.62,\ 0.45,\ 0.80,\ 1.0)$$——**$$z_2$$ 是险步**：走到这里成功率反而从 0.62 掉到 0.45（这步把链带偏了但还有救）。每步的增量价值 $$A(z_t) = V(z_t) - V(z_{t-1}) = (-0.17,\ +0.35,\ +0.20)$$：第一步后是坏步、二三步是好步——**PRM 就是推理链的 critic**（[贝尔曼方程](/ai-fa/explore/40-Concepts/贝尔曼方程) 的 $$V$$ 与优势在推理树上的化身）。
