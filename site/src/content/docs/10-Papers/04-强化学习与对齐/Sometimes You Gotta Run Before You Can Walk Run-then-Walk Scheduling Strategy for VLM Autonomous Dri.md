...
- 谱系锚：[GRPO与RLVR](/ai-fa/explore/20-Algorithms/GRPO与RLVR) · [GRPO目标](/ai-fa/explore/30-Formulas/GRPO目标)（本文=其奖励侧的两阶段课程化）· [策略梯度定理](/ai-fa/explore/40-Concepts/策略梯度定理)
- 奖励设计：分段终点奖励+门控安全与 [过程奖励与结果奖励（PRM-ORM）](/ai-fa/explore/40-Concepts/过程奖励与结果奖励（PRM-ORM）) 的"结果奖励整形"一族相接
- 同域对照：[ForeDrive](/ai-fa/explore/10-Papers/09-世界模型与JEPA/ForeDrive Foresight-Guided End-to-End Autonomous Driving with a Planning-Relevant Latent World Model)——同在 NAVSIM，ForeDrive 证明纯 IL+世界模型未来注入已到 89.9，本文走 RL 后训练路线并给出"先探索后修复"的课程序；两条线共同把"安全约束的引入时机"变成显式设计维度
- 线锚：[视觉语言模型（VLM）](/ai-fa/explore/40-Concepts/视觉语言模型（VLM）)
...
[GRPO目标](/ai-fa/explore/30-Formulas/GRPO目标)（组相对优势的形式）· [策略梯度定理](/ai-fa/explore/40-Concepts/策略梯度定理) · NAVSIM/PDMS 指标结构（NC/DAC/EP 等子项——Run 期选 PDMS 正因 EP 是其中最易优化的分量）· [过程奖励与结果奖励（PRM-ORM）](/ai-fa/explore/40-Concepts/过程奖励与结果奖励（PRM-ORM）)（奖励整形的背景谱系）。
