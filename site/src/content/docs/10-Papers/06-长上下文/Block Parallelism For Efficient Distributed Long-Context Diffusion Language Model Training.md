...
## 5. 与前作/矩阵关系
- 线锚：[FlashAttention分块计算](/ai-fa/explore/30-Formulas/FlashAttention分块计算)（分布式注意力的分块计算/通信本体）· [KV缓存](/ai-fa/explore/40-Concepts/KV缓存)（clean/corrupted KV 的跨卡通信量是全卡动机）
- 同族：↔ [Ring Attention](/ai-fa/explore/10-Papers/06-长上下文/Ring Attention with Blockwise Transformers for Near-Infinite Context（Ring Attention）)（上下文并行 CP 基线家族——本卡观察到 corrupted KV 无跨块复用，把按位置切分换成块所有权+上下文分片，260919 处理段挂链，占位层待精读）；跨线 [Diffusion Forcing](/ai-fa/explore/10-Papers/09-世界模型与JEPA/Diffusion Forcing- Next-token Prediction Meets Full-Sequence Diffusion（Diffusion Forcing）)（BDLM=块间 AR×块内扩散，正是 DF 结构在语言模型侧的系统化训练）
