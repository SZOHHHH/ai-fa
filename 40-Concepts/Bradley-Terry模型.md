...
**第 2 步：训练奖励模型（负对数似然）。** 标注员选了 A，模型当前给 $r_A=1,\ r_B=0.5$：损失 $= -\log\sigma(1-0.5) = -\log 0.622 \approx 0.475$。梯度方向：**拉大 $r_A - r_B$**（对 [[40-Concepts/sigmoid函数]] 卡"自适应刹车"的另一面：模型已分对且分差大时梯度自动趋零）。这就是 RLHF 奖励模型的全部训练公式（[[30-Formulas/RLHF目标]] §奖励模型段）。
...
不可算的配分函数消失了——剩下的全是策略对数比。**把这个式子当损失（偏好数据的 NLL）= DPO**（[[30-Formulas/DPO损失]]）：绕过奖励模型，直接在策略上优化偏好。理解了这四步，DPO 不再是"神奇公式"而是"BT+玻尔兹曼+消元"的三步推论。
...
4. BT 建不了"好多少"——哪个损失试图补这个洞？（KTO：前景理论的 gain/loss 非对称，见 [[30-Formulas/KTO损失]]）
