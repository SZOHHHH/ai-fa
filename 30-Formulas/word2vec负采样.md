...
- $\sigma$ = [[40-Concepts/sigmoid函数|sigmoid]]——每一项都是一次"真/假邻居"的二分类打分
...
**第 2 步：读懂公式的两半。**
- 正项 $\log\sigma(v_c^\top v_t)$：**真邻居**（分解↔低秩）的内积要大——"这对真是邻居"的置信度（内积=[[40-Concepts/内积]] 相似度）
...
**第 5 步：这张表就是嵌入空间。** 训练完的向量表 $E$ 即 [[40-Concepts/嵌入向量（Embedding）]]——"国王-男人+女人≈女王"的可算结构是**副产品**：任务只逼向量"能判邻居"，判邻居需要语义相似度 → 相似词必须向量靠近。谱系：SGNS（2013）→ 对比学习（InfoNCE 与本式同构，[[30-Formulas/CLIP对比损失]]）→ 一切"拉近正对/推远负对"的现代训练。
...
- 与 [[30-Formulas/交叉熵]] 的关系：每个二分类项 $\log\sigma(\cdot)$ 本身就是二类交叉熵（[[40-Concepts/sigmoid函数]] §3"负采样判别"行）
...
- [[40-Concepts/sigmoid函数]]：真/假邻居打分器
- [[40-Concepts/内积]]：相似度原语
- [[40-Concepts/嵌入向量（Embedding）]]：训练的表=嵌入本体
- [[40-Concepts/独热编码（One-Hot）]]：词编号的载体（查表视角）

## 7. 与其他公式的关系

- → 派生：整个 [[40-Concepts/嵌入向量（Embedding）]] 时代的起点（专门预训练流派的代表）
- ↔ 同构：[[30-Formulas/CLIP对比损失]]（图文对当"邻居"，InfoNCE 化的 SGNS）
- ↔ 对照：[[30-Formulas/交叉熵]]（每一项=二类 CE）
