...
**第 2 步：凸函数（平方）同一样本。** $f(x)=x^2$：$(\mathbb{E}[X])^2 = 50.5^2 = 2550$；$\mathbb{E}[X^2] = \frac{1+10000}{2}=5000.5$——**$5000.5 \ge 2550$ ✓**（凸函数：函数在期望之下），且差值 $= \mathrm{Var}(X) = 2450.5$（这不是巧合：$\mathbb{E}[X^2]-\mathbb{E}[X]^2$ 恒等于方差——**Jensen 的缺口本身就是方差**，[[40-Concepts/方差与协方差]]）。

**第 3 步：ELBO 里的一步（认出它）。** $\log\mathbb{E}_q[\frac{p(x,z)}{q}] \ge \mathbb{E}_q[\log\frac{p(x,z)}{q}]$——形态=第 1 步：左=真值 $\log p(x)$，右=ELBO，**缺口=随机性（用 q 采样带来的）折损**，且可以精确写成 $\mathrm{KL}(q\|p(z|x))$（[[40-Concepts/ELBO]] §2 第 5 步）。Jensen 不等式在哪，下界就在哪。
