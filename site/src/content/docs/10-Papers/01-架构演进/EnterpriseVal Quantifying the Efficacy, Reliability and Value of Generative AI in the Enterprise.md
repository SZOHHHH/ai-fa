...
## 5. 与前作/矩阵关系
- 线锚：[Transformer](/ai-fa/explore/20-Algorithms/Transformer)（被测对象基座） · [注意力核心公式](/ai-fa/explore/30-Formulas/注意力核心公式)
- 同族：[LLM-as-Judge](/ai-fa/explore/10-Papers/04-强化学习与对齐/Judging LLM-as-a-Judge with MT-Bench and Chatbot Arena（LLM-as-Judge）)（其评审协议=盲评专家×校准 LLM 评审经预测驱动推断扩表，正是该线的工程化放大） · [Hallucination Survey](/ai-fa/explore/10-Papers/04-强化学习与对齐/A Survey on Hallucination in Large Language Models（Hallucination Survey）)（引用精度/幻觉率作为部署门槛的度量背景）
