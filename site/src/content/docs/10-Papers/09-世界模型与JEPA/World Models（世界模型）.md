...
- **三件套架构**：V=[VAE](/ai-fa/explore/10-Papers/02-生成建模与扩散/Auto-Encoding Variational Bayes（VAE）) 把 64×64 像素压成 32 维隐向量；M=MDN-RNN 预测下一隐状态（**混合密度输出——天然表达多模态未来**，算法实体卡：[MDN-RNN](/ai-fa/explore/20-Algorithms/MDN-RNN)）；C=线性策略（只有几千参数）；
