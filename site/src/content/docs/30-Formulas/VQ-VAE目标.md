...
**第 4 步：直通估计（骗反向传播）。** 前向：解码器吃 $$z_q = 0$$；反向：$$\partial/\partial z_e$$ 被当恒等映射抄送——重建损失假装"解码器吃的是 1.5"来训练编码器。与 [重参数化](/ai-fa/explore/40-Concepts/重参数化) 同一动机（梯度穿过不可导层）：一个靠恒等捷径、一个靠路径拆分。
