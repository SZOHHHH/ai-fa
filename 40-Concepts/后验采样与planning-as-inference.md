...
**第 4 步：多模态现场（与 argmax 的分水岭）。** 若两条路都通（$p(x_{goal}|左) = p(x_{goal}|右) = 0.5$）→ 后验 $(0.5, 0.5)$——**不选边，两条路各留一半**；argmax/MAP 会随便挑一条（实现细节决定），把"我其实不确定"的信息全部丢弃。后验采样每次掷骰——探索与多样性免费获得（[[40-Concepts/贝叶斯公式]] 批 1 教程的 odds 拔河在此换成动作版）。

**第 5 步：怎么算这个后验（工程三路）。** ①参数化直接学（[[40-Concepts/逆动力学（IDM）]]：$q_\theta(a|s, s_{goal})$ 一步到位）；②引导采样（DPS：扩散先验 + 每步叠加 $\nabla\log p(y|x)$，无需重训）；③硬条件 inpainting（Diffuser：已知帧涂死、生成其余）——三路的取舍见 §3 工具箱。
