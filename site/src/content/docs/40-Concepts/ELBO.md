...
**形式**（两种等价形态，本页母版；实操见 [ELBO目标](/ai-fa/explore/30-Formulas/ELBO目标)）：
...
**第 3 步：Jensen 一步定乾坤。** $$\log$$ 是凹函数，$$\log\mathbb{E}[\cdot] \ge \mathbb{E}[\log\cdot]$$（[Jensen不等式](/ai-fa/explore/40-Concepts/Jensen不等式)）：
...
**第 6 步：手算一个迷你 gap。** 设真后验与替身都是一维高斯：$$p(z|x)=\mathcal{N}(1, 1)$$，$$q=\mathcal{N}(1.5, 2)$$。KL 闭式（两高斯，[KL散度](/ai-fa/explore/40-Concepts/KL散度)）：
...
**第 7 步：为什么叫"变分"。** 变分法=在**函数/分布空间**上做优化（不是在参数空间）。这里对整个分布 $$q_\phi$$ 求最优（"变分推断"），Kingma 把 $$q$$ 参数化为神经网络（amortized）——推断一次前向完成，于是有了 [VAE](/ai-fa/explore/20-Algorithms/变分自编码器)。

## 3. 为什么 AI 需要它（大一统视角）

| 出现场景 | 用法 |
|---|---|
| [VAE](/ai-fa/explore/20-Algorithms/变分自编码器) | 整个 VAE 的训练目标就是 ELBO（一步隐变量） |
| [DDPM训练目标](/ai-fa/explore/30-Formulas/DDPM训练目标) | 变分界是 ELBO 的马尔可夫链版本，逐项化简成 MSE |
...
- [KL散度](/ai-fa/explore/40-Concepts/KL散度)：正则项与 gap 项的双重身份
- [期望](/ai-fa/explore/40-Concepts/期望)：ELBO 全程是期望形式（蒙特卡洛估计）
- [重参数化](/ai-fa/explore/40-Concepts/重参数化)：让 ELBO 可梯度下降的关键技巧
- [Jensen不等式](/ai-fa/explore/40-Concepts/Jensen不等式)：下界成立的原因
- [ELBO目标](/ai-fa/explore/30-Formulas/ELBO目标)：实操版（损失形态、闭式 KL、表示对照表）
