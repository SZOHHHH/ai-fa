...
**第 5 步：统一框架买了什么。** DDPM（VP 脉）与 SMLD（VE 脉）原是两套记号两族论文；Score-SDE 证明它们只是 $$f, g$$ 的两个系数选择——**反向 SDE、概率流 ODE、Fokker-Planck 全套定理证一次、全族通用**（[反向SDE](/ai-fa/explore/30-Formulas/反向SDE)、[概率流ODE](/ai-fa/explore/30-Formulas/概率流ODE) 直接继承）。
