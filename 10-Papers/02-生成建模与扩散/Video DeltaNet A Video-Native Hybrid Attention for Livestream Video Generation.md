...
- ←线性注意力谱系直系：[[Gated DeltaNet-2- Decoupling Erase and Write in Linear Attention（GDN2）]]（门控与衰减机制的直接前身）· [[Parallelizing Linear Transformers with the Delta Rule over Sequence Length（DeltaNet并行）]]（delta 规则与并行化基础）——本文=把这套 LLM 侧机制做"视频原生"（帧粒度+双向+混合）
- ↔对照：[[40-Concepts/稀疏与线性注意力]]（稀疏化 vs 线性化两条降复杂度路线，本文取混合）· [[20-Algorithms/SSM序列架构（Mamba系）]]（同为固定状态压缩远上下文，选择性机制对照 delta 规则）
- 宿主与部署：[[Scalable Diffusion Models with Transformers（DiT）]]（改造对象）· 少步加速承 [[20-Algorithms/一致性模型]] 系蒸馏
- 概念/公式根基：[[30-Formulas/注意力核心公式]] · [[30-Formulas/注意力计算复杂度]] · [[30-Formulas/均方根归一化]]
...
[[30-Formulas/注意力核心公式]] 与 [[30-Formulas/注意力计算复杂度]]（为什么 Softmax 是瓶颈：MiniMax H3 工作负载中占去噪器 85%+ 时间）· [[40-Concepts/稀疏与线性注意力]]（外积记忆/核化注意力视角）· delta 规则线性注意力（先读 [[Gated DeltaNet-2- Decoupling Erase and Write in Linear Attention（GDN2）]]）· [[Scalable Diffusion Models with Transformers（DiT）]]。
