...
## 5. 与前作/矩阵关系
- ← 谱系：[[10-Papers/09-世界模型与JEPA/Genie 2- A Large-Scale Foundation World Model（Genie 2）|Genie 2]]（通用基础世界模型路线）、[[10-Papers/09-世界模型与JEPA/Genie- Generative Interactive Environments（Genie）|Genie]]（动作条件交互环境生成起点）。
- ↔ 像素级动作条件 WM：[[10-Papers/09-世界模型与JEPA/Diffusion for World Modeling- Visual Details Matter in Atari（DIAMOND）|DIAMOND]]（像素扩散 WM×游戏）——同"像素级动作条件生成"轴，域不同（机器人 vs Atari）。
- 蒸馏轴：[[40-Concepts/知识蒸馏]] + [[10-Papers/03-后处理/Distilling the Knowledge in a Neural Network（KD）|KD 奠基]]——其"35→4 步"属少步蒸馏族（同族的扩散少步化：一致性/渐进蒸馏谱系），概念卡蒸馏形态表可记一行"步数蒸馏×世界模型"。
- ↔ 下游评估用法：与 [[10-Papers/09-世界模型与JEPA/RodForesight - A World Model Enhanced Diffusion Policy for Slender and Material Agnostic Rod Insertion（杆件插装）|RodForesight]]（同日命中，WM 当预演评估器）互证"WM 评估策略"已成工程惯例——**而其评估有效的前提恰是 WM 保真未失：从反面提醒 E1 的问题意识（蒸馏后 WM 的决策保真塌缩）在具身域同样存在但被生成指标遮蔽**。
...
## 7. 读前须知
- 前置：视频预测 WM 基本结构（[[10-Papers/09-世界模型与JEPA/Genie- Generative Interactive Environments（Genie）|Genie]] 先看）、稀疏 MoE 直觉（[[10-Papers/05-MoE/DeepSeekMoE- Towards Ultimate Expert Specialization in Mixture-of-Experts Language Model（DeepSeekMoE）|DeepSeekMoE]]）、蒸馏基础（[[40-Concepts/知识蒸馏]]）。
