...
- 对齐谱系：[[20-Algorithms/DPO系（离线偏好优化）]] · [[30-Formulas/DPO损失]]（其混合离线/在线 DPO 阶段）· [[20-Algorithms/RLHF]] · [[30-Formulas/RLHF目标]]（奖励驱动的在线 RL 阶段，作用对象换成前向扩散过程）
- 蒸馏谱系：[[40-Concepts/知识蒸馏]] + [[CausalOPD- First-Wrong-Step Supervision for Distilling Causal Chain Reasoning（CausalOPD）]]（on-policy 蒸馏一族：在学生自己的分布上向教师/专家学习）——本文把"on-policy 蒸馏"用作**策略合并器**而非压缩器，是少见用法
- 底座：[[20-Algorithms/潜在扩散模型（LDM）]] / [[20-Algorithms/扩散模型]]（MMDiT 双流注意力骨干）· [[40-Concepts/视觉语言模型（VLM）]]（judge 与数据引擎的执行件）
...
[[30-Formulas/DPO损失]]（偏好优化阶段的语言）· [[40-Concepts/知识蒸馏]]（软目标与在线分布）· [[20-Algorithms/扩散模型]]（前向过程上做 RL 意味着把去噪轨迹当策略轨迹）· [[40-Concepts/视觉语言模型（VLM）]]（judge 评估器的偏差与自我迭代风险——报告用人工兜底关键检查点）。
