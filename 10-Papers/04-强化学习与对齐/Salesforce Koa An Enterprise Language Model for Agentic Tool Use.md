...
## 5. 与前作/矩阵关系
- 线锚：[[20-Algorithms/GRPO与RLVR]]（GRPO 后训练 Nemotron-3-Super-120B 开源底座）
- 近邻同族：[[DeepSeekMath- Pushing the Limits of Mathematical Reasoning in Open Language Models（DeepSeekMath）]]（GRPO 本源）· [[Tandem Reinforcement Learning with Verifiable Rewards（TRL）]]（可验证奖励后训练同族：Koa 的 task-resolution 奖励=Agent Script 规格展开的仿真任务接地 verifiable reward）
