...
- → 同族最强近亲：[Astronex-World](/ai-fa/explore/10-Papers/02-生成建模与扩散/Astronex-World 1.0 Real-Time Interactive World Model Foundation)（同底座同配方家族——块因果+KV 缓存+一致性初始化+Self-Forcing+++分布匹配，几乎同周发布；分岔在表征：Astronex 透视+动作接口，AlayaVista 全景状态×视口读出）；
- 方法链：块因果×扩散沿 [Diffusion Forcing](/ai-fa/explore/10-Papers/09-世界模型与JEPA/Diffusion Forcing- Next-token Prediction Meets Full-Sequence Diffusion（Diffusion Forcing）)；少步化沿 [DMD](/ai-fa/explore/10-Papers/02-生成建模与扩散/One-step Diffusion with Distribution Matching Distillation（DMD）)→[DMD2](/ai-fa/explore/10-Papers/02-生成建模与扩散/Improved Distribution Matching Distillation for Fast Image Synthesis（DMD2）)→本卡分布匹配；一致性蒸馏初始化 ↔ [一致性模型](/ai-fa/explore/10-Papers/02-生成建模与扩散/Consistency Models（一致性模型）)；同属 [NFE（函数求值次数）](/ai-fa/explore/40-Concepts/NFE（函数求值次数）) 压缩家族；
- 家族对照：[Genie 2](/ai-fa/explore/10-Papers/09-世界模型与JEPA/Genie 2- A Large-Scale Foundation World Model（Genie 2）)（基础 WM 闭源轴）/[DIAMOND](/ai-fa/explore/10-Papers/09-世界模型与JEPA/Diffusion for World Modeling- Visual Details Matter in Atari（DIAMOND）)（游戏域像素扩散 WM——决策侧对应物）；
...
- → 后继补记（260922）：[WorldCrafter: Consistent Video World Model with Implicit 3D-aware Memory](/ai-fa/explore/10-Papers/02-生成建模与扩散/WorldCrafter Consistent Video World Model with Implicit 3D-aware Memory)——"透视+记忆 vs 显式 3D vs 全景潜状态"三角里的**隐式 3D 记忆**新代表：新视角重建预训练编码器写记忆+按目标视角读出固定预算 token（重访一致性 +47.6%）；仍是生成质量轴，无决策接口。

## 7. 读前须知
- **前置**：[扩散模型](/ai-fa/explore/20-Algorithms/扩散模型)、[流匹配](/ai-fa/explore/20-Algorithms/流匹配)（速度参数化）、[条件流匹配损失](/ai-fa/explore/30-Formulas/条件流匹配损失)；
- **按需**：[NFE（函数求值次数）](/ai-fa/explore/40-Concepts/NFE（函数求值次数）)（双分支各 4 步）、[KV缓存](/ai-fa/explore/40-Concepts/KV缓存)（块因果缓存流式）、[位置编码](/ai-fa/explore/40-Concepts/位置编码)（SpheRoPE=RoPE 的球面推广）、[DMD](/ai-fa/explore/10-Papers/02-生成建模与扩散/One-step Diffusion with Distribution Matching Distillation（DMD）)（分布匹配前置）；
