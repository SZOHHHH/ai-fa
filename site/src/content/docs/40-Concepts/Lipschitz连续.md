...
**第 4 步：神经网络的 L 从哪来。** 复合网络 $$f = W_L\phi(\cdots\phi(W_1 x))$$：每层贡献 $$\le\sigma(W_l)$$（线性层）×激活的 L（ReLU=1）——**全网 L ≤ ∏ σ(W_l)**。谱归一化把每层除到 1 → 全网 ≤1（[谱归一化](/ai-fa/explore/30-Formulas/谱归一化) §2 第 1 步的完整逻辑链）；梯度惩罚则直接罚 $$\|\nabla_x f\|$$（L 的逐点替身，[梯度惩罚](/ai-fa/explore/30-Formulas/梯度惩罚)）。
