...
**第 1 步：五点表。** $\sigma(-2) = 0.119$、$\sigma(-1) = 0.269$、$\sigma(0) = 0.5$、$\sigma(1) = 0.731$、$\sigma(2) = 0.881$——对称性验证：$\sigma(1)+\sigma(-1) = 0.731+0.269 = 1$ ✓（$\sigma(-x) = 1-\sigma(x)$）。

**第 2 步：导数自封闭。** $\sigma'(x) = \sigma(1-\sigma)$：峰值 $\sigma'(0) = 0.25$；$\sigma'(\pm2) = 0.881\times0.119 = 0.105$（峰值的 42%）；$\sigma'(\pm4) = 0.982\times0.018 = 0.0177$（**峰值的 1/14**）——饱和区梯度近乎断流，这是 sigmoid 让出隐藏层激活席位的数值原因。

**第 3 步：同一函数在 DPO 里的另一个身份。** 批 6 的数字：$z = 0.6$（DPO 内层）→ $\sigma = 0.646$；批 1 BT 卡：$\sigma(2) = 0.881$（偏好概率）——sigmoid 从"激活函数"转岗"概率出口"后，饱和不是缺陷而是**刹车**：DPO 梯度权重 $\sigma(\hat r^- - \hat r^+)$ 在模型判断已对时（$z$ 大）自动趋零。

**第 4 步：sigmoid = 二类 softmax 验证。** $\mathrm{softmax}(2, 0) = (e^2, e^0)/(e^2+1) = (0.881, 0.119) = (\sigma(2), \sigma(-2))$ ✓——只对差敏感（平移不变），天然配"两物比较"（BT/DPO 的 $\sigma(r^+ - r^-)$ 正是这个差）。
...
1. $\sigma(2)$ 与 $\sigma(-2)$？（0.881 与 0.119——和为 1 的镜像对）
2. 导数峰值在哪、多大？（$x=0$ 处 0.25；$\sigma'(\pm4)$ 只剩 0.0177（1/14）——饱和断流）
3. 饱和在 DPO 里为什么反而是优点？（梯度权重 $\sigma(\hat r^- - \hat r^+)$ 随"已学对"自动趋零——自适应刹车）
4. sigmoid 与二类 softmax 的关系？（$\mathrm{softmax}(z_1,z_2) = (\sigma(z_1{-}z_2), \sigma(z_2{-}z_1))$——只对差敏感）
