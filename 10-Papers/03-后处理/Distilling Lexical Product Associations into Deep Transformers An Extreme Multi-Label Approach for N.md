...
- ←直接使用 [[DistilBERT, a distilled version of BERT- smaller, faster, cheaper and lighter（DistilBERT）]] 作编码器（应用其压缩产物）
- ≡蒸馏源对照 [[Distilling the Knowledge in a Neural Network（KD）]]：经典 KD 蒸的是大模型的暗知识，此处蒸的是**统计量**（词法关联）——"教师不必是模型"的变体
- ↔[[40-Concepts/知识蒸馏]]：概念的应用侧新案例（统计教师）
