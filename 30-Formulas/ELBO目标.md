...
最大化 ELBO ⇔ 同时把"解码误差"压小、"编码分布"拉向先验。推导与 gap 的精确身份见概念母页 [[40-Concepts/ELBO]]（§2 七步慢推）。
...
代入验证（$d{=}1$，$\mu{=}2$，$\sigma{=}3$）：$\frac{1}{2}(4+9-2.197-1)\approx 4.90$ ✓（手算见 [[20-Algorithms/变分自编码器]] §3 第 4 步）。**训练时这一项不用采样、精确可算**——它只是网络输出的初等函数。

**第 3 步：重建项化成你认识的损失。** 重建项 $\mathbb{E}_q[\log p_\theta(x|z)]$ 的形态由解码器的**分布假设**决定（[[40-Concepts/最大似然估计（MLE）]] 出身）：
...
| GMM 参数（[[20-Algorithms/MDN-RNN]]） | **混合 NLL** | 序列/隐状态预测 |
...
一次前向：$x\to(\mu,\sigma)\to z\to\hat x$；KL 精确、重构项单样本蒙特卡洛（样本量=1 通常就够——重参数化梯度方差小的红利，[[40-Concepts/重参数化]]）。
...
- 母页概念：[[40-Concepts/ELBO]]（七步推导+gap 手算）
- ⊃ **泛化为** [[30-Formulas/DDPM训练目标]]（把 $z$ 换成整条马尔可夫链再化简）
- ⊃ **泛化为** [[30-Formulas/条件流匹配损失]]（连续时间、速度场参数化下的 ELBO）
- ↔ **条件先验版**：[[30-Formulas/潜变量变分下界]]（先验 $p(z|u)$ 带辅助变量）
- ≡ **等价于** [[20-Algorithms/变分自编码器]] 的完整损失（实操形态见本卡 §2）
