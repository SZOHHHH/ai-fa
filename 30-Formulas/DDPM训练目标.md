...
**第 2 步：前向造题。** 闭式一步（玩具宇宙见 [[30-Formulas/DDPM前向过程]]）：$x_2 = 0.985\times2 + 0.173\times0.5 = 1.970+0.086 = 2.056$——**网络的考题**：看见 $(x_2, t)$，猜当初加的 $\epsilon$。

**第 3 步：网络答卷与判分。** 网络（还没学好）输出 $\epsilon_\theta(x_2, 2) = 0.6$。损失 $= (0.6-0.5)^2 = 0.01$（真实按维平均，玩具一维）。

**第 4 步：梯度往哪推。** $\partial L/\partial\epsilon_\theta = 2(\epsilon_\theta - \epsilon) = +0.2$——把预测往 0.5 压。**没有对抗、没有博弈**——纯回归，这是扩散训练比 GAN 稳定的结构原因（对照 [[30-Formulas/GAN目标]]）。
