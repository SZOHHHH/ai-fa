...
- **可适配存量 WM**：能把 [HY-WorldPlay](/ai-fa/explore/10-Papers/09-世界模型与JEPA/WorldPlay- 长期几何一致的实时交互世界建模（HY-WorldPlay）) 迁到新数据分布/新延迟目标；
...
蒸馏段即 Causal Forcing 的非对称 DMD（本卡是框架卡，公式见 [Causal Forcing](/ai-fa/explore/10-Papers/09-世界模型与JEPA/Causal Forcing- 自回归扩散蒸馏的正确姿势（Causal Forcing）)）：
...
- ← 集大成 [Causal Forcing](/ai-fa/explore/10-Papers/09-世界模型与JEPA/Causal Forcing- 自回归扩散蒸馏的正确姿势（Causal Forcing）)/++ 蒸馏配方 + WorldPlay 系基座；
...
[NFE（函数求值次数）](/ai-fa/explore/40-Concepts/NFE（函数求值次数）)；流式推理与 KV 缓存；MMDiT 与 cross-attention 两种条件注入的差别。
