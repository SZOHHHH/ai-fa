...
**第 1 步：ODE 步没有抽签。** 接玩具宇宙：$x_2 = 2.056$，$\epsilon_\theta = 0.5$ → $\hat{x}_0 = 2.000$（[[30-Formulas/DDPM训练目标]] 第 5 步的反解）。PF-ODE 的欧拉步 = DDIM $\eta=0$ 更新：$x_1 = \sqrt{\bar\alpha_1}\,\hat{x}_0 + \sqrt{1-\bar\alpha_1}\,\epsilon_\theta = 0.995\times2 + 0.1\times0.5 = 2.040$——**没有 $z$、没有随机性**：同样的 $(x_2, \epsilon_\theta)$ 永远走同一条路。

**第 2 步：与 SDE 采样对照。** [[30-Formulas/反向SDE]] 同起点抽 $z = 0.5$ 得 2.060；ODE 得 2.040——**两种采样都合法**：SDE 输出是分布的一个样本（带那一步噪声 $\sqrt{\tilde\beta}\,z$），ODE 输出是同边缘分布的确定性轨迹点。
...
**第 4 步：三大红利的钥匙都是"确定性"。** ①精确似然：瞬时换元公式沿轨迹积分（连续归一化流视角）；②可逆双射 $x_0 \leftrightarrow x_T$：编辑/插值/inversion 的基础（图像反演出 latent、改条件重生）；③少步采样：轨迹平滑才敢大步跳——步数计量单位即 NFE（[[40-Concepts/NFE（函数求值次数）|NFE]]）。

**第 5 步：FM 的位置。** [[30-Formulas/条件流匹配损失]] 干脆直接回归这个右端（速度场 $v_\theta = dx/dt$）——**流匹配 = 换个预测量训练同一个 PF-ODE**：训练推导与扩散平行而更简洁。

## 3. 直觉解释

- **同一枚硬币的两面**：SDE 采样（随机、多样）与 ODE 采样（确定、可逆）共享同一边缘分布——想换随时换，模型不用重训
- **ODE 的三大红利**：①精确对数似然（连续归一化流视角）②隐空间插值（起终点编码）③少步采样（好积分器 + 直轨迹；步数的计量单位即 NFE，见 [[40-Concepts/NFE（函数求值次数）|NFE]]）
