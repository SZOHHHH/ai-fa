...
- GLU 系：门与货**分家**——$\sigma$ 型门只管开关（0~1），货路线性保真——"**控制流与数据流分离**"（思想同 [[20-Algorithms/LSTM]] 的门与传送带、[[30-Formulas/MoE门控公式]] 的路由与专家）
- SwiGLU=门函数选 Swish（比 sigmoid 门/GLU 门在实验里略好，Shazeer 2020 的消融）

**第 3 步：为什么现代 LLM 全用它。** LLaMA/Qwen/DeepSeek 系标配——经验上比 ReLU/GELU FFN 稳定增益约 1% 量级；真正决定性的是**门控线性结构**（分家带来的表达力），Swish 与否是次级选择。配套 [[30-Formulas/均方根归一化]]+RoPE 构成"LLaMA 配方三件套"（[[20-Algorithms/Transformer]] §5 现代化组件行）。
...
| GEGLU | GELU | 2020 | 平滑门 |
| **SwiGLU（本卡）** | Swish | [[GLU Variants Improve Transformer（SwiGLU）]] | LLM 标配 |
...
- [[40-Concepts/sigmoid函数]] 在门控位的长寿（激活函数族 §4："位置决定选择"）

## 5. 数学概念分解

- [[40-Concepts/sigmoid函数]]：Swish 的门芯
- [[30-Formulas/GELU激活]]：近亲（概率门 vs sigmoid 门）
- [[40-Concepts/激活函数族]]：族谱（门控线性终点）
- [[40-Concepts/内积]]：三路投影全是线性层

## 6. 与其他公式的关系

- ← 前身：标准 FFN（ReLU）+GLU（2017）
- → 现代配方：与 [[30-Formulas/均方根归一化]]、RoPE 组成 LLaMA 系三件套
- ↔ 同思想：[[30-Formulas/MoE门控公式]]（粗粒度门：选专家）、[[20-Algorithms/LSTM]]（时间维门）
