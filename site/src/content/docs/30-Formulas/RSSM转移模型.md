...
- **重参数化贯穿**：$$s$$ 采样一律 $$s=\mu+\sigma\odot\epsilon,\ \epsilon\sim\mathcal{N}(0,I)$$（见 [重参数化](/explore/40-Concepts/重参数化)）——随机性外置，梯度才能穿。
...
| 论文 | 贡献 |
|---|---|
| [PlaNet 2019](/explore/10-Papers/09-世界模型与JEPA/Learning Latent Dynamics for Planning from Pixels（PlaNet）) | RSSM 提出 + CEM 规划 |
| [Dreamer 2020](/explore/10-Papers/09-世界模型与JEPA/Dream to Control- Learning Behaviors by Latent Imagination（Dreamer）) | RSSM 上挂 actor-critic（想象训练） |
| [DreamerV3 2023](/explore/10-Papers/09-世界模型与JEPA/Mastering Diverse Domains through World Models（DreamerV3）) | 分类潜状态+三稳定化，跨域通吃 |

## 5. 数学概念分解

- [状态空间模型方程](/explore/30-Formulas/状态空间模型方程)：RSSM=它的循环版（加了确定链与动作条件）
- [重参数化](/explore/40-Concepts/重参数化)：随机状态的梯度通路
- [KL散度](/explore/40-Concepts/KL散度)：先验贴推断的胶水
- [马尔可夫链](/explore/40-Concepts/马尔可夫链)：$$s_t$$ 只依赖 $$(h_t)$$ 的马氏结构

## 6. 与其他公式的关系

- ↔ 对照 [DDPM前向过程](/explore/30-Formulas/DDPM前向过程)/像素扩散系：**latent 压缩 vs 像素生成**两大世界模型路线的公式分界
- → 应用：[贝尔曼最优方程](/explore/30-Formulas/贝尔曼最优方程) 的价值递归在 RSSM 的 $$s_{1:T}$$ 上展开（Dreamer 系的梦内学习）
- → 对偶：[逆动力学（IDM）](/explore/40-Concepts/逆动力学（IDM）) 反推的 $$(s,s')$$ 在 RSSM 系里即 $$(s_t, s_{t+k})$$ 潜状态对
