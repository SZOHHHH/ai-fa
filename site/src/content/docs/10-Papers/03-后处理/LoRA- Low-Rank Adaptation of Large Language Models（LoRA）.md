...
- [LoRA分解](/explore/30-Formulas/LoRA分解) —— 本文灵魂

## 5. 与前作的关系

- 超越了 [Parameter-Efficient Transfer Learning for NLP](/explore/10-Papers/03-后处理/Parameter-Efficient Transfer Learning for NLP（Adapter）)（串行延迟）与 [Prefix-Tuning - Optimizing Continuous Prompts for Generation](/explore/10-Papers/03-后处理/Prefix-Tuning- Optimizing Continuous Prompts for Generation（Prefix-Tuning）)（占上下文）
- 理论根基 [低秩分解](/explore/40-Concepts/低秩分解)；结构哲学同 [残差连接](/explore/30-Formulas/残差连接)
