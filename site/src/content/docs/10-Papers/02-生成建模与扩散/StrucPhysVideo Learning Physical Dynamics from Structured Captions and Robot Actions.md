...
## 5. 与前作/矩阵关系
- 线锚：[扩散模型](/ai-fa/explore/20-Algorithms/扩散模型) · [混合专家（MoE）](/ai-fa/explore/20-Algorithms/混合专家（MoE）)（稀疏 MoE TI2V 骨干）· [NFE（函数求值次数）](/ai-fa/explore/40-Concepts/NFE（函数求值次数）)（IA2V 交互 rollout 少步蒸馏到 4 步去噪）
- 近邻同族：[UniSim](/ai-fa/explore/10-Papers/09-世界模型与JEPA/Learning Interactive Real-World Simulators（UniSim）)（动作条件交互视频模拟器同族，本文多走一步"少步蒸馏落 rollout"）
