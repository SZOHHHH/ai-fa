...
**形式**：分词器=双射映射 $\text{text} \leftrightarrow (\text{token 序列}) \to (\text{编号序列})$，再查 [[40-Concepts/嵌入向量（Embedding）]] 表进模型：
...
| 场景 | 用法 | 库内链接 |
|---|---|---|
| LLM 入口 | 一切文本的必经第一站 | [[20-Algorithms/Transformer]] |
| 训练目标 | next-token 预测=预测"下一个编号" | [[40-Concepts/最大似然估计（MLE）]]（序列 MLE） |
| 上下文长度 | 上下文按 token 计数 | [[40-Concepts/KV缓存]] |
| 采样生成 | 从词表分布里挑编号、解码回文本 | [[40-Concepts/采样器]] |
...
- [[40-Concepts/嵌入向量（Embedding）]]：编号→向量的下一站
- [[40-Concepts/独热编码（One-Hot）]]：编号的数学载体（查表视角）
- [[20-Algorithms/Transformer]]：消费 token 序列的主干
- [[40-Concepts/采样器]]：生成侧的出口（token 分布→文本）
- [[40-Concepts/KV缓存]]：上下文以 token 为单位计价与缓存
