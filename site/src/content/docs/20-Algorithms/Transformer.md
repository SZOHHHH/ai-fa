...
**第 1 步：入口。** 分词（[Tokenization（分词）](/ai-fa/explore/40-Concepts/Tokenization（分词）)）→ 查嵌入表得向量（[嵌入向量（Embedding）](/ai-fa/explore/40-Concepts/嵌入向量（Embedding）)）→ 加位置编码（[RoPE旋转位置编码](/ai-fa/explore/30-Formulas/RoPE旋转位置编码)：第 5 步的"毫米尺/千米尺"）。

**第 2 步：注意力层（灵魂）。** "追"作为 query 回看全场：打分 $$q^\top k/\sqrt{d_k}$$——[注意力核心公式](/ai-fa/explore/30-Formulas/注意力核心公式) 教程用这两个字手算过全程（"追"对"猫"的权重如何压倒对"追"自己）；softmax 后加权取 V（[softmax函数](/ai-fa/explore/40-Concepts/softmax函数) 的 (2,1,0) 手算）；多头分工（[注意力机制](/ai-fa/explore/40-Concepts/注意力机制) 六步）。

**第 3 步：FFN 层。** SwiGLU 门控（[SwiGLU门控](/ai-fa/explore/30-Formulas/SwiGLU门控)：门×货一维手算 −0.269×5）——注意力负责"收集信息"，FFN 负责"加工信息"（占参数 2/3）。

**第 4 步：胶水与堆叠。** 残差（[残差连接](/ai-fa/explore/30-Formulas/残差连接)：三块 0.5 链 0.125 vs 1.875——梯度高速公路）+ RMSNorm（[均方根归一化](/ai-fa/explore/30-Formulas/均方根归一化)）——一个块走完，叠 L 层。

**第 5 步：出口。** 顶层向量投影到词表 logits（设 (2,1,0) 形态）→ softmax 出下一 token 分布（[温度参数](/ai-fa/explore/40-Concepts/温度参数) 三档手算：T=1 得 0.680 采样或 argmax）。

**第 6 步：生成循环。** 预测的 token 拼回输入，重复——KV 缓存让历史投影只算一次（[KV缓存](/ai-fa/explore/40-Concepts/KV缓存)：7B 每请求 2.15GB 的账单）。**每一步的数学都住在一公式卡里，本卡是地图。**
