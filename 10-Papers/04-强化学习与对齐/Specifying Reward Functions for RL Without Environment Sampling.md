---
type: paper
title: Specifying Reward Functions for RL Without Environment Sampling
aliases: []
year: 2026
authors: [Stephane, Hatgis-Kessell]
venue: 待核（占位层，来自 arXiv 2026-09-14）
arxiv: "2609.15544v1"
pdf: 已下载（PDF/）
line: 强化学习与对齐
matrix_coords: 待评
tags: [paper, 占位层]
layer: 占位
---
# Specifying Reward Functions for RL Without Environment Sampling

> **占位层卡**（daily 自动采集 2026-09-16 建卡，元数据出自 arXiv API=已核实）。本链 Claude 处理段将自动精读升级：七节补全+中文速览+挂全链。
> 摘要（原文）：Enabling human stakeholders to specify reward functions that lead to their desired outcomes is a key challenge in deploying reinforcement learning agents. Preference-based methods such as online RLHF can reduce the burden of manual reward design, but they require repeatedly training policies, sampling trajectories from the real world, and eliciting feedback, making them impractical in settings where environment interaction is computationally expensive or unsafe. We introduce Experience-Free Autonomous Reward Specification (EARS), a method for learning reward functions from preferences without 

## 1. 一句话贡献
（待精读）

## 2. 核心贡献
- （待精读）

## 3. 方法概要
（待精读）

## 4. 核心公式
（待精读）

## 5. 与前作/矩阵关系
- 线锚：[[20-Algorithms/RLHF]]
- 数学根基：[[40-Concepts/Bradley-Terry模型]]（想象轨迹对的偏好→特征权重，BT 似然照用）· [[30-Formulas/RLHF目标]]
- 近邻同族：[[Deep reinforcement learning from human preferences（RLHF）]]（在线偏好 elicitation 本源——EARS 把"采样真轨迹"换成"在 LLM 构造的特征空间采想象轨迹"，偏好学习循环不变）

## 6. 影响后续
（待精读）

## 7. 读前须知
（待精读）自动下载备注：已下载（PDF/）
