...
- **无重建潜模型**：编码器把观测压进 512 维潜空间，转移模型在潜空间预测，监督只有三项——潜 TD 一致性、奖励预测、潜空间一致性（相邻编码别漂移）——**没有像素重建项**（与 Dreamer 系的根本区别，承 [MuZero](/explore/10-Papers/09-世界模型与JEPA/Mastering Atari, Go, Chess and Shogi by Planning with a Learned Model（MuZero）) 的价值等价哲学）；
...
- 需要的前置：[贝尔曼方程](/explore/40-Concepts/贝尔曼方程)、[TD误差与自举](/explore/40-Concepts/TD误差与自举)、[MuZero](/explore/10-Papers/09-世界模型与JEPA/Mastering Atari, Go, Chess and Shogi by Planning with a Learned Model（MuZero）)（哲学源头）

## 5. 与前作/矩阵关系

- ← TD-MPC（2022，初版：同骨架更小规模）← MuZero（价值等价哲学）；
- ↔ 三分天下教学组：[Dreamer](/explore/10-Papers/09-世界模型与JEPA/Dream to Control- Learning Behaviors by Latent Imagination（Dreamer）)（重建+策略式）/ TD-MPC2（价值等价+搜索式）/ [DIAMOND](/explore/10-Papers/09-世界模型与JEPA/Diffusion for World Modeling- Visual Details Matter in Atari（DIAMOND）)（像素扩散+策略式）；
...
- **必前置**：[贝尔曼方程](/explore/40-Concepts/贝尔曼方程)、[TD误差与自举](/explore/40-Concepts/TD误差与自举)、[MuZero](/explore/10-Papers/09-世界模型与JEPA/Mastering Atari, Go, Chess and Shogi by Planning with a Learned Model（MuZero）)；
...
> 数学根基：[贝尔曼方程](/explore/40-Concepts/贝尔曼方程) · [贝尔曼最优方程](/explore/30-Formulas/贝尔曼最优方程)
